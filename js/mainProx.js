...
if (gh) {
    (function(window, document) {
        ...
        const main = (event) => {
            ...
            if (event['type'] == 'click') {
                if (event.composedPath()['length'] == 12) {
                    ...
                    settings.elePrint.classList.remove('bg-success')
        ...
        const imprimir = () => {
            ...
            textEncoded += '--------------------------------\n'
            textEncoded += ' Data:                  ' + defaults.printHJ + '\n'
            textEncoded += ' Hora:                     ' + defaults.printHr + '\n'
            textEncoded += '--------------------------------\n'
            textEncoded += ' Data:                  ' + defaults.printSorteio + '\n'
            textEncoded += ' Hora:                     17:00\n'
            textEncoded += '--------------------------------\n'
            textEncoded += ' Cambista:          ' + localStorage.getItem('cambista') + '\n'
            textEncoded += ' Código:            ' + defaults.printCode + '\n'
            textEncoded += '--------------------------------\n'
            textEncoded += ' Número:\n'
        ...
        const elPrint = (event) => {
            ...
            if (hora == defaults.eleHoraFinal && minuto < defaults.eleMinutoFinal && settings.eleDate.value == hj) {
                ...
                if (disponivel) {
                    ...
                    defaults.printCode = text
                    defaults.printHJ = hjPrint
                    defaults.printHr = String(hora + ':' + minuto)
                    defaults.printSorteio = sorteio

                    settings.elePrint.classList.add('bg-success')
                    settings.elePrint.classList.remove('bg-danger')
                    settings.elePrint.disabled = false
                    settings.eleCheck.classList.remove('bg-success')
                    settings.eleCheck.classList.add('bg-danger')
                    settings.eleCheck.disabled = true
                ...
                if (settings.eleDate.value > hj) {
                    ...
                    if (disponivel) {
                        ...
                        settings.eleCheck.classList.remove('bg-success')
                        settings.eleCheck.classList.add('bg-danger')
                        settings.eleCheck.classList.add('text-white')
                        settings.eleCheck.disabled = true
        ...
        const setUp = () => {
            ...
            settings.eleCheck = defaults.eleMain.querySelector(defaults.eleCheck)
        ...
        const events = function() {
            ...
            settings.elePrint.addEventListener('click', imprimir)
            settings.eleCheck.addEventListener('click', elPrint)
